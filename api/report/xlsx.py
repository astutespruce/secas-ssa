...
from api.report.nlcd import add_ncld_impervious_sheet, add_ncld_landcover_sheet
...
def create_xlsx(df, datasets):
    ...
    area_col_width = max(
        df.overlap_acres.apply(lambda x: len("{x:,.2f}")).max() * CHAR_PER_WIDTH_UNIT,
        10,
    )
    ...
    with pd.ExcelWriter(buffer) as xlsx:
        ...
        if "landfire_evt" in datasets:
            add_landfire_evt_sheet(xlsx, df, name_col_width, area_col_width, area_label)
